"""
训练曲线对比图生成脚本
方案A（加载预训练权重）与方案B（不加载预训练权重）对比
输出: training_loss_comparison.png, training_accuracy_comparison.png
"""
...
ax1.plot(epochs, loss_0, 'o-', color=C0, linewidth=1.0, markersize=3.5,
         markerfacecolor=C0, markeredgewidth=0.5, markeredgecolor='white', label='方案A')
ax1.plot(epochs, loss_1, 's-', color=C1, linewidth=1.0, markersize=3.5,
         markerfacecolor=C1, markeredgewidth=0.5, markeredgecolor='white', label='方案B')
...
ax2.plot(epochs, train_acc_1, 's-',  color=C1, linewidth=1.0, markersize=3.5,
         markerfacecolor=C1, markeredgewidth=0.5, markeredgecolor='white', label='方案B 训练准确率')
...
plt.close()
print('已保存: training_accuracy_comparison.png')
