...
plt.rcParams['font.size'] = 7.5  # 六号字

# A4 可打印宽度的一半，使图片占 A4 宽度约 50%，可并排或单列插入
FIG_WIDTH = 3.15
...
all_labels = []
all_probs = []
# 存储有趣样本的图片用于可视化分析，避免存储全量数据
# 格式: {global_idx: (image_tensor_cpu, true_label, pred_label, confidence)}
sampled_images = {}
...
plt.savefig('confusion_matrix_percentage.eps', format='eps', bbox_inches='tight')
print("百分比混淆矩阵已保存: confusion_matrix_percentage.png / .eps")
...
print("混淆矩阵图片已保存: confusion_matrix.png / .eps")
...
# ── 敏感性 Sensitivity 和 特异性 Specificity 计算 ──
...
# 选有代表性的类别绘制 ROC 曲线：准确率最高 3 类和最低 3 类
...
plt.savefig('roc_curve.eps', format='eps', bbox_inches='tight')
print("ROC 曲线已保存: roc_curve.png / .eps")
...
plt.savefig('per_class_accuracy.eps', format='eps', bbox_inches='tight')
print("每类准确率图已保存: per_class_accuracy.png / .eps")
...
plt.savefig('confidence_distribution.eps', format='eps', bbox_inches='tight')
...
plt.savefig('support_vs_accuracy.eps', format='eps', bbox_inches='tight')
...
plt.savefig('confusion_top_confusions.eps', format='eps', bbox_inches='tight')
print("主要混淆类别热力图已保存: confusion_top_confusions.png / .eps")
...
SAMPLES_PER_GRID = 6  # 每张网格图展示 6 个案例，2 行 x 3 列，文字清晰

def save_case_grid(indices, title, filename, max_samples=SAMPLES_PER_GRID):
    """
    从 sampled_images 中查找索引对应的图片并生成网格
    只使用已保存了图片的索引，不会出现空白框
    """
    ...
    eps_name = os.path.splitext(filename)[0] + '.eps'
    plt.savefig(eps_name, format='eps', bbox_inches='tight')
    plt.close()
    print(f"已保存: {filename} / {eps_name}")

# 改用 sampled_images 中的索引而非全局索引
if len(sampled_images) >= 6:
    # 从 sampled_images 中按置信度排序
...
fig.savefig('metrics_summary.eps', format='eps', bbox_inches='tight')
plt.close()
print("指标汇总表已保存: metrics_summary.png / .eps")
...
print(f"\n生成的文件:")
files = [
    "confusion_matrix.png", "confusion_matrix_percentage.png",
    "per_class_accuracy.png", "confidence_distribution.png",
    "support_vs_accuracy.png", "confusion_top_confusions.png",
    "roc_curve.png", "metrics_summary.png",
    "classification_report.txt", "confusion_matrix.csv",
    "error_high_confidence.png", "correct_low_confidence.png",
    "correct_high_confidence.png", "error_typical_cases.png",
]
for f in files:
    if os.path.exists(f):
        print(f"  {f}")
